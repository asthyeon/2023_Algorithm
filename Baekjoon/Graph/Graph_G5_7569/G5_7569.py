...
def bfs(q, arr):
    ...
    if q:
        ...
        for dx, dy in [[0, 1], [1, 0], [0, -1], [-1, 0]]:
            ...
            if wall * N <= nx < N + (wall * N) and 0 <= ny < M and arr[nx][ny] == 0:
                q.append((nx, ny))
        ...
        for dx in [N, -N]:
            hx = sx + dx
            # 벽 형성 및 이동할 수 있는 곳이라면
            if 0 <= hx < N * H and arr[hx][sy] == 0:
                q.append((hx, ny))
                arr[hx][ny] = arr[sx][sy] + 1
        check += 1


# 상자 가로 칸: M, 상자 세로 칸: N, 상자의 수: H
